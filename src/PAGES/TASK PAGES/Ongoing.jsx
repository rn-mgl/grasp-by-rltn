...
export default function Ongoing() {
  ...
  const fetchClassTask = React.useCallback(async () => {
    setLoading(true);
    try {
      const { data } = await axios.get(`${url}/tasks/ongoing`, {
        headers: { Authorization: token },
      });
      if (data) {
        setOngoingTask(data.ongoing_data);
      }
    } catch (error) {
      console.log(error);
  ...
  return (
    ...
    >
      ...
      {ongoingTask?.length > 0 ? (
        <TasksPreviewBar task={ongoingTask} />
      ) : (
        <MessagePage
          header={"No Ongoing Tasks"}
          body={"your ongoing tasks will be displayed here"}
